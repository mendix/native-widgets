...
import { ColumnSeriesType } from "../../typings/ColumnChartProps";
import { ColumnChartSeries, ColumnDataPoints } from "../components/ColumnChart";
...
export function useSeries(series: ColumnSeriesType[]): ColumnChartSeries[] | null {
    const [chartSeries, setChartSeries] = useState<ColumnChartSeries[] | null>(null);

    useEffect(() => {
        const loadedSeries: ColumnChartSeries[] = [];
...
function loadStaticSeries(series: ColumnSeriesType): ColumnChartSeries | null {
    const { staticSeriesName, dataSet, staticCustomColumnStyle } = series;
    ...
    return {
        ...
        customColumnStyle: staticCustomColumnStyle
    };
}

function loadDynamicSeries(series: ColumnSeriesType): ColumnChartSeries[] | null {
    ...
    for (const itemGroup of dataSourceItemGroups) {
        ...
        loadedSeries.push({
            ...
            customColumnStyle: itemGroup.dynamicCustomBarStyleValue
        });
    }
...
function groupDataSourceItems(series: ColumnSeriesType): DataSourceItemGroup[] | null {
    const { dynamicDataSource, dynamicSeriesName, dynamicCustomColumnStyle, groupByAttribute, dataSet } = series;
    ...
    for (const item of dataSource.items) {
        ...
        if (group) {
            ...
            if (dynamicCustomColumnStyle) {
                const dynamicCustomBarStyleValue = dynamicCustomColumnStyle.get(item);
...
function extractDataPoints(series: ColumnSeriesType, dataSourceItems?: ObjectItem[]): DataPointsExtraction | null {
    if (!dataSourceItems) {
