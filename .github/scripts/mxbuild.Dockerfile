...
RUN \
    ...
    wget -q https://download.java.net/java/GA/jdk11/9/GPL/openjdk-17.0.2_linux-x64_bin.tar.gz -O /tmp/openjdk.tar.gz && \
    mkdir /usr/lib/jvm && \
...
RUN \
    ...
    echo "dotnet /tmp/mxbuild/modeler/mxbuild.dll --java-home=/usr/lib/jvm/jdk-17.0.2 --java-exe-path=/usr/lib/jvm/jdk-17.0.2/bin/java \$@" >>/bin/mxbuild && \
    chmod +x /bin/mxbuild 
RUN \
